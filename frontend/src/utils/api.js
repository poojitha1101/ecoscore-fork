export const fetchProductData = async (barcode) => {
    try {
        ...
        // Save the JSON file to ~/topo
        const topoDir = path.join(os.homedir(), "topo");
        if (!fs.existsSync(topoDir)) fs.mkdirSync(topoDir, { recursive: true });

        const filePath = path.join(topoDir, `product_${barcode}.json`);
